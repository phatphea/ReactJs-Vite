...
function App() {
  ...
  return (
    <section className="max-w-screen-xl mx-auto mt-16 px-5">
      {/* categories */}
      <section className="mt-20 w-full">
        <div className="flex overflow-x-auto gap-3 pb-2 hide-scrollbar w-[95%] mx-auto mb-5">
          {categories.map((category, index) => (
            <button
              onClick={()=>handleClick(category.slug)}
              key={index}
              className="flex-shrink-0 px-4 py-2 bg-teal-50 text-teal-900 hover:bg-teal-200 rounded text-sm transition-all cursor-pointer"
            >
              {category.name}
            </button>
          ))}
